...
class Test1_StairsProgressive(unittest.TestCase):
    def test_GetID(self):
        self.assertTrue(Progressive.GetID(), const.ID.STAIRS_SIMPLE)

class Test2_StairsProgressive(unittest.TestCase, StandartStrategy):
    def setUp(self):
    ...
    def test_SaveAndRestore_from_string(self):
        ...
        restore_stairs = Progressive.RestoreFromRecoveryString(recovery_string)
...
class Test3_StairsProgressive(unittest.TestCase, StandartStrategy):
    def setUp(self):
        StandartStrategy.__init__(self)
        self._save_filepath = os.path.join(faf.SplitPath1(sys.argv[0]), "stairs-Progressive.save_file.log")
...
class Test4_StairsProgressive(unittest.TestCase, Test_Srairs):
    def setUp(self):
...
class Test5_StairsProgressive(unittest.TestCase, Test_Srairs):
    def setUp(self):
        Test_Srairs.__init__(self, Progressive(), "StairsProgressive-USDT_RUB.log")
        self._stairs.SetAvailableCurrency(100000)
        self._stairs.SetCommissionBuy(0.9988)
        self._stairs.SetCommissionSell(0.9988)
        self._stairs.SetCoefficient(3)
        self._stairs.SetPricePrecision1(3)
        self._stairs.SetQuantityPrecision1(8)
        self._stairs.SetProfit(1.01)
