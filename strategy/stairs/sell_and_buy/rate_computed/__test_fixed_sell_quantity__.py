...
from strategy.stairs.sell_and_buy.rate_computed import SbRcFixedSellQuantityS

class Test4(SrairsStandartTest):
    def setUp(self):
        SrairsStandartTest.setUp(self)
        self._stairs = SbRcFixedSellQuantityS()
    ...
    def test1(self):
        self.assertEqual(SbRcFixedSellQuantityS.GetID(), const.ID.SbRcFixedSellQuantityS)

    # brief: testing a creation of strategy preview
    def test2(self):
        save_strategy_filepath = os.path.join(faf.SplitPath1(__file__), "SbRcFixedSellQuantityS.log")
