...
class StrategyPreview:
    ...
    def _StairsStrategyPreviewWithSubsteps(self):
        log.Logger.RegisterRecipient(repr(self), self._ResultPreviewwriter, True)
        PreviewLogger = lambda init_message: log.Logger(init_message, recipient=repr(self))
        with PreviewLogger(["Global info:", self._strategy.GetStep()]) as logger:
            key = s_const.INFO.GLOBAL
        ...
        while is_next_step_save:
            with PreviewLogger(["Step {}:", self._strategy.GetStep()]) as logger:
                ...
                with PreviewLogger([]) as sub_logger:
                    ...
                    sub_logger.LogInfo("buy cost = {}", i_step[key.BUY_COST])
                    sub_logger.LogInfo("buy rate = {}", i_step[key.BUY_RATE])
                    with PreviewLogger("Substeps:") as _:
                        subkey = key.SUBINFO
                        i_substep = i_step[subkey]
                        for i, substep in enumerate(i_substep):
                            with PreviewLogger(["Substep {}:", i+1]) as substep_logger:
                                substep_logger.LogResult("diff rate = {}", substep[subkey.DIFF_RATE])
                                substep_logger.LogResult("diff cost = {}", substep[subkey.DIFF_COST])
                                substep_logger.LogResult("sell rate = {}", substep[subkey.SUBRATE_SELL_RATE])
                                substep_logger.LogResult("sell cost = {}", substep[subkey.SUBRATE_SELL_COST])
                                substep_logger.LogResult("sell profit = {}", substep[subkey.SUBRATE_SELL_PROFIT])
                                substep_logger.LogResult("sell quantity = {}", substep[subkey.SUBRATE_SELL_QUANTITY])
                                substep_logger.LogResult("buy rate = {}", substep[subkey.SUBRATE_BUY_RATE])
                                substep_logger.LogResult("buy cost = {}", substep[subkey.SUBRATE_BUY_COST])
                                substep_logger.LogResult("buy quantity = {}", substep[subkey.SUBRATE_BUY_QUANTITY])
            try:
                self._strategy = self._strategy.ComputeNextFullStep()
            except s_error.ExceededAvailableCurrency as eac_error:
                with PreviewLogger(["Waiting-values for trade-strategy:", self._strategy.GetStep()]) as logger:
                    logger.LogResult("sell-quantity = {}", eac_error.GetSellQuantity())
                    logger.LogResult("sell-cost = {}", eac_error.GetSellCost())
    ...
    def GetPreview(self):
        ...
        if II(ss.SimpleSubsteps):
            self._StairsStrategyPreviewWithSubsteps()
        elif II(ss.Simple):
